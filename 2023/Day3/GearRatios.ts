...
const readFile = async (filePath: string): Promise<string> =>
	await fsReadFile(resolve(__dirname, filePath), 'utf8').catch((error) => {
...
const getResults = (input: string[]): { part1: number; part2: number } => {
	...
	const numbers = input.flatMap((item, y) => {
		let indexOffset = 0;
		const match = item.match(/\d+/g);
		return !match
			? []
			: match.map((num) => {
					const index = item.indexOf(num, indexOffset);
					indexOffset = index + num.length;
					return { x: index, y, num: +num, used: false };
			  });
	});
	...
	input.forEach((item, y) => {
		[...item].forEach((char, x) => {
			...
			if (/[^\d.]/.test(char)) {
				numbers.filter((v) => {
					const valid = isNumberInRange(x, v.x - 1, v.x + String(v.num).length) && isNumberInRange(v.y, y - 1, y + 1) && !v.used;
					valid && (v.used = true);
